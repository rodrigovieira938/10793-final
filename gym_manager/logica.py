...
class Logica:
    ...
    def CriarInstructor(self, nome : str, telefone : str, email : str, morada : str, idade : int):
        ...
        return user
    def CriarAulaDeGrupo(self,instrutor : Utilizador, horainicio : datetime.datetime, horafinal : datetime.datetime, limit : int):  # um instrutor não pode dar mais de uma aula em simultâneo 
        aula = Aulasgrupo(None, instrutor, horainicio, horafinal, limit)
        
        for al in self.db.ListartAulaDeInstrutor(instrutor):
            if aula.horainicio <= al.horafinal and al.horafinal <= aula.horafinal:
                return False
        
        aula.id = self.db.CriarAulaDeGrupo(instrutor, horainicio.strftime("%d/%m/%Y %H:%M"), horafinal.strftime("%d/%m/%Y %H:%M"), limit)
        return aula
