from .content import Content, curses, PAGINA_ALUNO
    ...
    def input(self, ch):
        if ch == curses.KEY_DOWN and self.vscrollbar.max > 0:
            ...
            if len(self.alunos) == 0:
                return
            self.content.pages[PAGINA_ALUNO].aluno = self.alunos[self.vscrollbar.current]
            self.content.preswitch(PAGINA_ALUNO)
            self.content.tui.windows[TAB_WINDOW].selected_before = self.content.tui.windows[TAB_WINDOW].selected
            self.content.tui.windows[TAB_WINDOW].selected = PAGINA_ALUNO
            self.content.switch(PAGINA_ALUNO)
    def render(self):
